<?php
  ...
  $stat = "SELECT `AEBux`,`rank` FROM `Users` WHERE `email` = '$user'";
    if($stat != ""){
...
?>
		    ...
		    <a href="#nav"> <span class="glyphicon glyphicon-th-list"> </span></a>
				<span style="position: absolute; right: 40px; top: 10px;font-size:11px">Rank: <?php echo $rank; ?> </span>
		  		<span style="position: absolute; right: 40px;  top: 26px; font-size:11px">AEBuxs: <?php echo $aebux; ?> </span>
		  		<span style="position: absolute; right: 40px;  top: 41px; font-size:11px"> <a style="background-color: inherit;
